import {Component, effect, inject, signal, OnDestroy, Input} from '@angular/core';
import {Firestore, collection, collectionData, query, where} from '@angular/fire/firestore';
import {Subscription} from 'rxjs';
import {MonthlySummary} from '../../../app/core/model/cashbook.model';
import {DecimalPipe, NgClass} from '@angular/common';
import {MonthlyChartComponent} from '../../../app/admin/monthly-chart/monthly-chart';
import {CashbookService} from '../../../app/core/services/cashbook.service';

@Component({
  selector: 'app-emonthlyanalysis',
  imports: [
    NgClass,
    DecimalPipe,
      ...
      <div class="report-card">
        ...
        <div class="header-section">
          <div class="year-selector">
            ...
            <select id="yearSelect" [value]="year()" (change)="onYearChange($event)" class="year-dropdown">
...
export class Emonthlyanalysis implements OnDestroy {
  ...
  loadReport() {
    ...
    // Query Firestore directly for entries of the selected year (removed orderBy to avoid index requirement)
    ...
    this.subscription = collectionData(q, { idField: 'id' }).subscribe({
      ...
      error: (err: any) => {
        ...
        this.error.set(err.message || 'Failed to load report data');
        this.loading.set(false);
      }
    });
  }
  ...
  private generateMonthlyReport() {
    ...
    const report = months.map(month => {
      const monthEntries = this.allEntries.filter(e => e.month === month);
      ...
      return {
        month,
        monthName: new Date(this.year(), month - 1).toLocaleString('default', { month: 'long' }),
        receipts,
        payments,
